 ...
 * Pre-conditions:
 * 	
 * 		1. There should exist at least one optional disjunctive filter not included in the test case.
 * 
 ...
 * 
 * This transformation creates a single follow-up test case by adding a random number of disjunctive filters to the source test case.
 * 
 ...
 *
 */
public class DisjunctiveExtendTransformation extends AbstractInputExtendTransformation {

	public DisjunctiveExtendTransformation(SUT sut) {
		super(sut);
	}
	...
	protected List<TestParameter> getSelectableParameters(TestCase testCase) {
		...
		parameters.addAll(sut.getDisjunctiveFilters().stream().filter(p -> !tcParameters.contains(p))
